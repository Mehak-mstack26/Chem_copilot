...
# import requests # Keep for potential future API calls - currently unused directly in app.py
import re
# import sys # sys import seems unused, can be removed if not needed elsewhere << REMOVED
import os
import api_config # Ensures API key is set from environment, used by ChatOpenAI
from langchain_community.callbacks.streamlit import StreamlitCallbackHandler
from langchain_openai import ChatOpenAI
...
# Import from test.py
from test import enhanced_query, tools as agent_tools, clear_chatbot_memory, add_to_chatbot_memory, run_chatbot_query # Added clear_chatbot_memory, add_to_chatbot_memory, run_chatbot_query

# --- NameToSMILES tool setup for app.py's direct use ---
name_to_smiles_tool_instance = None
for tool_in_agent_list in agent_tools:
    if tool_in_agent_list.name == "NameToSMILES":
        name_to_smiles_tool_instance = tool_in_agent_list
        break

if name_to_smiles_tool_instance is None:
    st.error("Fatal Error: NameToSMILES tool not found. Name conversion will not work.")
...
st.markdown("""
    ...
    .assistant-message {background-color: #f0f4c3; margin-right: 20%;}
    </style>
    """, unsafe_allow_html=True)
...
if 'user_query_input' not in st.session_state:
    st.session_state.user_query_input = ""
# For enhanced_query path (more structured analysis)
if 'structured_query_to_analyze' not in st.session_state:
    st.session_state.structured_query_to_analyze = ""
if 'original_user_provided_name' not in st.session_state:
    st.session_state.original_user_provided_name = None
if 'analyzed_smiles_structured' not in st.session_state:
    st.session_state.analyzed_smiles_structured = None
if 'structured_analysis_result' not in st.session_state:
    st.session_state.structured_analysis_result = None
if 'show_structured_analysis_section' not in st.session_state:
    st.session_state.show_structured_analysis_section = False

# For chatbot path
if "messages" not in st.session_state:
    st.session_state.messages = [] # Stores {role: "user/assistant", content: "message"}
...
def get_smiles_from_name_app(name):
    if name_to_smiles_tool_instance is None:
        ...
        return None
    try:
        result_str = name_to_smiles_tool_instance.run(name)
        match = re.search(r"SMILES:\s*([^\s]+)", result_str, re.IGNORECASE)
        if match:
            smiles = match.group(1).strip()
            if re.match(r"^[A-Za-z0-9@\[\]\.\+\-\=\#\:\(\)\\\/;\$\%\|\{\}]+$", smiles) and ">>" not in smiles:
...
ui_mode = st.radio(
    ...
    ('💬 Chat with ChemCopilot', '🔬 Structured Analysis '),
    key="ui_mode_selection"
)
st.markdown("---")


if ui_mode == '💬 Chat with ChemCopilot':
    st.markdown("## ChemCopilot Chat")

    # Display chat messages
    for message in st.session_state.messages:
        with st.chat_message(message["role"]):
            st.markdown(message["content"])
            if message["role"] == "assistant" and "visualization_path" in message and message["visualization_path"]:
                if os.path.exists(message["visualization_path"]):
                    st.image(message["visualization_path"])
                else:
                    st.caption(f"Visualization image not found: {message['visualization_path']}")


    chat_input = st.chat_input("Ask ChemCopilot about chemistry...", key="chat_main_input")
    ...
    if chat_input:
        ...
        with st.chat_message("assistant"):
            ...
            # Use StreamlitCallbackHandler for agent's thoughts in chat
            # This might be tricky if run_chatbot_query doesn't stream intermediate steps well.
            # For now, let's focus on the final response.
            # st_callback_chat = StreamlitCallbackHandler(message_placeholder, collapse_completed_thoughts=True)

            with st.spinner("ChemCopilot is thinking..."):
                # Call run_chatbot_query which uses the conversational agent with memory
                response_data = run_chatbot_query(chat_input) # callbacks=[st_callback_chat]
            
            ai_response_text = response_data.get("analysis", "Sorry, I couldn't process that.")
            viz_path = response_data.get("visualization_path")

            message_placeholder.markdown(ai_response_text)
            if viz_path and os.path.exists(viz_path):
                st.image(viz_path)
            elif viz_path:
                 st.caption(f"Visualization note: {viz_path}")
    ...
    if st.button("Clear Chat History", key="clear_chat_btn_chatmode"):
        st.session_state.messages = []
        clear_chatbot_memory() # Crucial: clear the agent's backend memory too
        st.rerun()


elif ui_mode == '🔬 Structured Analysis ':
    st.markdown("## Compound & Reaction Analysis ")

    user_input_structured = st.text_input(
        "Enter Compound Name, SMILES, Reaction SMILES, or a specific analysis query:",
        value=st.session_state.user_query_input, # Re-use if desired or make separate
        placeholder="e.g., 'full information about Glucose', 'CCO', 'analyze CC(=O)Cl.OCCO>>CC(=O)OCCO.Cl'",
        key="structured_query_input_main"
    )

    if st.button("Submit for Structured Analysis", key="submit_structured_query_button"):
        if user_input_structured:
            st.session_state.user_query_input = user_input_structured # Update if shared
            st.session_state.structured_query_to_analyze = user_input_structured
            st.session_state.show_structured_analysis_section = True
            st.session_state.structured_analysis_result = None
            st.session_state.analyzed_smiles_structured = None
            st.session_state.original_user_provided_name = None
            # Clear chatbot's memory if switching modes or starting fresh structured analysis
            clear_chatbot_memory()
        else:
            st.warning("Please enter a query for structured analysis.")

    if st.session_state.show_structured_analysis_section and st.session_state.structured_query_to_analyze:
        st.markdown('<div class="analysis-section">', unsafe_allow_html=True)
        st.markdown("## Structured Analysis Workspace")

        query_for_processing = st.session_state.structured_query_to_analyze
        original_name_for_query_param = None
        ...
        if full_info_name_match:
            ...
            if is_likely_name:
                ...
                if smiles:
                    ...
                    st.session_state.analyzed_smiles_structured = smiles
                    st.session_state.original_user_provided_name = subject_name_or_smiles
                    original_name_for_query_param = subject_name_or_smiles
                else:
                    st.warning(f"Could not convert '{subject_name_or_smiles}' to SMILES. Proceeding with the original query.")
        
        st.markdown("### Query for Structured Analysis Agent")
        st.code(query_for_processing)
        
        if st.session_state.original_user_provided_name:
            st.markdown(f"**Original Name Context:** `{st.session_state.original_user_provided_name}`")
        if st.session_state.analyzed_smiles_structured:
             st.markdown(f"**Initial Target SMILES:** `{st.session_state.analyzed_smiles_structured}`")

        if not st.session_state.structured_analysis_result:
            with st.spinner("ChemCopilot (Structured Analysis) is thinking..."):
                callback_container = st.container()
                st_callback = StreamlitCallbackHandler(callback_container)
                
                try:
                    # enhanced_query uses ZeroShot for general, Conversational for no-SMILES.
                    # For structured analysis, we primarily expect SMILES-based or specific tool calls.
                    result = enhanced_query(
                        query_for_processing, 
                        callbacks=[st_callback],
                        original_compound_name=original_name_for_query_param 
                    )
                    st.session_state.structured_analysis_result = result

                    if 'processed_smiles_for_tools' in result:
                        processed_smiles = result['processed_smiles_for_tools']
                        if processed_smiles:
                            st.session_state.analyzed_smiles_structured = processed_smiles
                    
                    if st.session_state.analyzed_smiles_structured:
                        st.markdown(f"**SMILES Processed by Agent:** `{st.session_state.analyzed_smiles_structured}`")
                    
                    # If this structured analysis yields a result, we might want to add it to the chatbot's memory
                    # if the user switches to chat mode and asks follow-ups.
                    # This makes the transition smoother.
                    if result.get("analysis"):
                         add_to_chatbot_memory(query_for_processing, result.get("analysis"))


                except Exception as e:
                    st.error(f"An error occurred during structured analysis: {str(e)}")
                    import traceback
                    with st.expander("See error details"):
                        st.code(traceback.format_exc())
        
        if st.session_state.structured_analysis_result:
            viz_path = st.session_state.structured_analysis_result.get('visualization_path')
            if viz_path and isinstance(viz_path, str) and not viz_path.lower().startswith("error"):
                if os.path.exists(viz_path):
                    st.markdown("### Visualization")
                    st.image(viz_path, caption="Chemical Visualization")
            ...
            st.markdown("### Structured Analysis / Answer")
            st.markdown('<div class="result-area">', unsafe_allow_html=True)
            st.markdown(st.session_state.structured_analysis_result.get('analysis', 'No analysis available or an error occurred.'))
            st.markdown('</div>', unsafe_allow_html=True)
            
            # Follow-up for structured analysis could transition to chat mode or use enhanced_query again
            # For simplicity here, we'll keep it within structured or suggest switching to chat.
            st.markdown("---")
            st.info("For conversational follow-ups on this analysis, switch to 'Chat with ChemCopilot' mode. The context of this analysis has been added to the chat memory.")

        if st.button("Clear Structured Analysis / New Query", key="clear_structured_analysis_btn"):
            st.session_state.show_structured_analysis_section = False
            st.session_state.structured_query_to_analyze = ""
            st.session_state.analyzed_smiles_structured = None
            st.session_state.original_user_provided_name = None
            st.session_state.structured_analysis_result = None
            # clear_chatbot_memory() # Decide if clearing chatbot memory here is desired.
                                 # Keeping it allows carry-over of context.
            st.rerun()
